...
GAL_NH = 1.39e22  #cm-2
SPIN = 0.998      #0.0, 0.5, 0.9, 0.99, 0.998

#66 degree inclination
#POL_FILE_PATH = os.path.join(XIMPOL_CONFIG, 'ascii',
#                             'bh_spin/Polarization_spin%s.txt'%(SPIN))
#30% of reflection
POL_FILE_PATH = os.path.join(XIMPOL_CONFIG, 'ascii',
                             'bh_spin/Polarization_spin%s_r30.txt'%(SPIN))

print "Using path", POL_FILE_PATH
FLUX_FILE_PATH = os.path.join(XIMPOL_CONFIG, 'ascii',
                              'bh_spin/Flux_spin%s.txt'%(SPIN))
...
_pol_degree = _pol_degree[_mask]/100.

fmt = dict(xname='Energy [keV]', yname='Polarization degree')
pol_degree_spline = xInterpolatedUnivariateSpline(_energy, _pol_degree, k=1,
                                                  **fmt)

_pol_angle = 270. - _pol_angle[_mask]
fmt = dict(xname='Energy', yname='Polarization angle [$^\\circ$]')
pol_angle_spline = xInterpolatedUnivariateSpline(_energy, _pol_angle, k=1,
                                                 **fmt)
...
_energy = _energy[_mask]
_flux = _flux[_mask]
_flux = scale_flux(_energy, _flux)

fmt = dict(xname='Energy', xunits='keV', yname='Flux',
           yunits='cm$^{-2}$ s$^{-1}$ keV$^{-1}$')
_energy_spectrum = xInterpolatedUnivariateSpline(_energy, _flux, k=1, **fmt)
